...
public class ArmSubsystem extends SubsystemBase implements AutoCloseable {
  ...
  public ArmSubsystem(Hardware armHardware) {
    ...
    this.absoluteEncoder = armHardware.absoluteEncoder;
  ...
  private void initializeArm() {
    ...
    armOffset =
        RobotBase.isReal()
            ? getAbsoluteAngle()
            : Units.radiansToDegrees(ArmConstants.ARM_OFFSET_RADS);
    DataLogManager.log("Arm Offset Data: " + armOffset);
  ...
  private void initMotor() {
    ...
    motorConfig.inverted(ArmConstants.INVERTED);
  ...
  public void periodic() {
    ...
    SmartDashboard.putNumber("Arm/Combined Angle", Math.toDegrees(getMeasurement()));
    SmartDashboard.putNumber("Arm/Encoder Delta", (getRelativeAngle() - getAbsoluteAngle()));
    ...
    SmartDashboard.putBoolean("Arm/Encoder Valid", absoluteEncoderValid);
    SmartDashboard.putBoolean("Arm/At Goal", atGoalPosition());
  ...
  // Command to move arm to extended position if not already there, or to retracted position if
  // already extended.
  public Command extendArmCommand() {
    return moveToPosition(Constants.ArmConstants.ARM_EXTENDED_RADS).withName("Extend Arm");
  }

  public Command retractArmCommand() {
    return moveToPosition(Constants.ArmConstants.ARM_RETRACTED_RADS).withName("Retract Arm");
  }
  ...
  public Command holdPosition() {
    return run(() -> {}).withName("Arm: Hold Position");
  }

  /** Abort Command will set the arm position to the goal position in any restricted areas. */
  ...
  /** Returns the relative arm angle using the built in encoder. The units are in degrees */
  public double getRelativeAngle() {
    return Math.toDegrees(encoder.getPosition() + ArmConstants.ARM_OFFSET_RADS);
  }
  ...
  /** Returns the absolute arm angle. The units are in degrees */
  public double getAbsoluteAngle() {
    // Add the offset from the 0 point of the encoder and wrap to within +/-180
    double angle = absoluteEncoder.getPosition() * 360 - ArmConstants.ABSOLUTE_OFFSET_DEGREES;
    if (angle >= 180.0) {
      angle -= 360.0;
    } else if (angle < -180.0) {
      angle += 360;
